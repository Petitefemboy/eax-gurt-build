...
ulong_t __stdcall Client::init( void* arg ) {
	...
	// welcome the user.
	g_notify.add( tfm::format( XOR( "welcome to eax, %s\n" ), g_cl.m_user ) );
...
void Client::DrawHUD( ) {
	...
	std::string text = tfm::format( XOR( "the | %s" ), m_user);
...
void Client::UpdateInformation( ) {
	...
	// are we in thirdperson, if so apply angle
	if( g_csgo.m_input->CAM_IsThirdPerson( ) ) 
		*reinterpret_cast< ang_t* >( uintptr_t( g_cl.m_local ) + 0x31C4 + 0x4 ) = m_angle;
