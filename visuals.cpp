...
void Visuals::Hitmarker() {
	...
	constexpr int a{ 4 };
	constexpr int b{ a + 5 };
	auto color = Color(225, 225, 225, alpha);
...
void Visuals::StatusIndicators() {
	...
	if (g_menu.main.misc.esp_style.get() == 0) {
		...
		if (g_aimbot.m_fake_latency || g_aimbot.m_fake_latency2) {
			...
			Color r = g_aimbot.m_fake_latency2 ? 150 : 255;
			Color g = g_aimbot.m_fake_latency2 ? 200 : 165;
			Color b = g_aimbot.m_fake_latency2 ? 60 : 0;
...
void Visuals::ManualAntiAim() {
	...
	// why would we render manual indicators like how it's done in skeet.idb? no point in rendering it as pixels
	if (g_hvh.m_left) {

		static int texture = g_csgo.m_surface->CreateNewTextureID(true);
		unsigned char buffer[4] = { 255, 255, 255, 255 };

		static Vertex verts[5] = {
			vec2_t(x - 40, y),
			vec2_t(x - 35, y + 7),
			vec2_t(x - 50, y),
			vec2_t(x - 35, y - 7),
		...
		g_csgo.m_surface->DrawSetColor(color);
		g_csgo.m_surface->DrawSetTexture(texture);
		g_csgo.m_surface->DrawTexturedPolygon(5, verts2);
	}
		...
		g_csgo.m_surface->DrawSetColor(color);
		g_csgo.m_surface->DrawSetTexture(texture);
		g_csgo.m_surface->DrawTexturedPolygon(5, verts3);
	}
	...
	// draw molotov.
		...
		if (model) {
			...
			if (name.find(XOR("flashbang")) != std::string::npos)
				render::esp_small2.string(screen.x, screen.y, col, XOR("FLASH"), render::ALIGN_CENTER);
				...
				render::esp_small2.string(screen.x, screen.y, col, XOR("FRAG"), render::ALIGN_CENTER);
			}
		}
	}
	// find classes.
		...
		if (spawn_time > 0.f && g_menu.main.visuals.proj.get()) {
			...
			render::esp_small2.string(screen.x - 13 + 26 * factor, screen.y + 7, col, tfm::format(XOR("%.1f"), (spawn_time + 7.031) - g_csgo.m_globals->m_curtime), render::ALIGN_CENTER);
			render::esp_small2.string(screen.x, screen.y, col, XOR("FIRE"), render::ALIGN_CENTER);
		}
	}
		...
		if (spawn_time > 0.f && g_menu.main.visuals.proj_range.get()) {
			...
			// render our timer in seconds and our title text
			render::esp_small2.string(screen.x - 13 + 26 * factor, screen.y + 7, col, tfm::format(XOR("%.1f"), (spawn_time + 18.04125) - g_csgo.m_globals->m_curtime), render::ALIGN_CENTER);
			render::esp_small2.string(screen.x, screen.y, col, XOR("SMOKE"), render::ALIGN_CENTER);
		}
	}
}
...
void Visuals::DrawItem(Weapon* item) {
	...
	auto font = g_menu.main.misc.esp_style.get() == 1 ? render::esp_small : render::esp_small2;

	if (g_menu.main.visuals.items.get() )
		font.string(screen.x, screen.y, Color(col.r(), col.g(), col.b(), alpha1), name, render::ALIGN_CENTER);
...
void Visuals::DrawPlayer( Player* player ) {
	...
	auto font = g_menu.main.misc.esp_style.get() == 0 ? render::esp2 : render::esp;
	auto font_small = g_menu.main.misc.esp_style.get() == 0 ? render::esp_small2 : render::esp_small;
	...
	if (name_esp) {
		...
		if (dormant)
			font.string(box.x + box.w / 2, box.y - font_small.m_size.m_height, Color(210, 210, 210, low_alpha), name, render::ALIGN_CENTER);
		else
			font.string(box.x + box.w / 2, box.y - font_small.m_size.m_height, Color(clr.r(), clr.g(), clr.b(), g_menu.main.players.name_esp_alpha.get()).alpha(low_alpha), name, render::ALIGN_CENTER);
	...
	if( health_esp ) {
		...
		if (player->m_iHealth() <= 92) {
			font_small.string(box.x - 5, y + (h - fill) - 5, { 255, 255, 255, low_alpha }, std::to_string(hp), render::ALIGN_CENTER);
		}
	}
	...
	// draw flags.
	{
		...
		for( auto it = items.begin( ); it != items.end( ); ++it ) {
			...
			if (*it == 6 && enemy && data->m_records.size() > 0) {
				...
				if (current->m_body != current->m_eye_angles.y) {
					if (dormant)
		...
		if (enemy && !dormant) {
			if (data->m_is_cheese_crack && !data->m_is_kaaba && !data->m_is_dopium) 
					flags.push_back({ XOR("CHEESE"), { 255, 200, 0, low_alpha } });
			

			if (data->m_is_kaaba && !data->m_is_cheese_crack && !data->m_is_dopium) 
					flags.push_back({ XOR("KAABA"), { 105, 200, 0, low_alpha } });
			

			if (data->m_is_dopium && !data->m_is_cheese_crack && !data->m_is_kaaba) 
					flags.push_back({ XOR("DOPIUM"), { 0, 195, 235, low_alpha } });
			

			if (data->m_is_eax) {
				Color col = Color(210, 210, 210, low_alpha);

				if (g_menu.main.misc.whitelist.get())
					col = Color(155, 210, 80, low_alpha);

				flags.push_back({ XOR("EAX"), col });
			}
			...
			if (data->m_is_pandora) 
					flags.push_back({ XOR("PANDORA"), { 166, 147, 243, low_alpha } });
			

			if (data->m_is_fade) 
					flags.push_back({ XOR("FADE"), { 240, 130, 0, low_alpha } });
			

			if (data->m_is_godhook) 
					flags.push_back({ XOR("GODHOOK"), { 220, 0, 0, low_alpha } });
			
		}


		if (data && data->m_records.size() && enemy)
		{
			LagRecord* current = data->m_records.front().get();
		...
		for (size_t i{ }; i < flags.size(); ++i) {
			...
			int offset = i * (font_small.m_size.m_height) + 1;
			font_small.string(box.x - 1 + box.w + 3, box.y + 1 + offset - 2, f.second, f.first);
		}
	}
				...
				if( data ) {
					...
					if (max != -1 && g_menu.main.players.ammo.get()) {
						...
						if (current > 0 && current <= int(std::floor(float(max) * 0.9f)) && !reload)
							font_small.string(box.x + bar, box.y + box.h + offset, { 255, 255, 255, low_alpha }, std::to_string(current), render::ALIGN_CENTER);
					...
					if (g_menu.main.players.weapontext.get()) {
						...
						if (dormant)
							font_small.string(box.x + (box.w / 2), box.y + box.h + offset, { 255, 255, 255, low_alpha }, name, render::ALIGN_CENTER);
						else
							font_small.string(box.x + (box.w / 2), box.y + box.h + offset, { 255, 255, 255, 210 }, name, render::ALIGN_CENTER);
						
						offset += font_small.m_size.m_height;
					}
