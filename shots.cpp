...
void Shots::OnShotFire(Player* target, float damage, int bullets, LagRecord* record, vec3_t aim_point, int hitbox, int hitgroup) {
	...
	if (target && record) {
		...
		if(success)
			g_notify.add(tfm::format("fired shot at %s | aim_point[%s:%i:%s], hc[%i], mode[%s:%i], bt[%i], vel[%i:%i], lag[%s:%s], lc[%s:%s], exp[%s]\n", 
				info.m_name, 
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	// hitmarker stuff lol
	g_visuals.m_hit_duration = 0.25f;
...
void Shots::OnShotMiss(ShotRecord& shot) {
	...
	if (!shot.m_record->m_bones) {
		//g_notify.add( "missed shot due to invalid record\n" );
		return;
	}
